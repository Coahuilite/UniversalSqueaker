...
- Not covered by any gate, and the likeliest in-game surprise: kernel widgets still return constant row heights in most `Measure()` paths, so the G1 text-clipping root cause is still open.
