...
- Old-UI removal decision, legacy-volume measurements, relocation set, capability gaps and two open correctness items are recorded in "Architecture audit and old-UI removal decision (2026-09-02)"; that section supersedes the Gate U "fallback must be retained / no clean cutover" constraint in the 2026-08-31 and 2026-09-01 checkpoints.
...
- Verified corrections to long-standing project statements: reflective HAR discovery is **not implemented** (`Catalog/SqueakXenotypeCatalog.cs:77` `TODO(HAR)`, assembled-only, `HarHintDefNames` permanently empty), so the `AGENTS.md` line about HAR being touched via reflection describes intent, not the shipped build. `CompProperties_Squeaker` carries only `actions` and `moodMods`. The kernel registers 16 `us/*` kinds, not 17.
- Two open correctness items (fixed together with the removal): `SqueakRuntimeResolver.NormalizeMode` silently degrades any unrecognised `SqueakVoicePackMode` to Vanilla with no assertion or log; and the `Disabled` bypass lives only on the production trigger path, so `CompSqueaker.PreviewFinal` ignores the mode and can emit audio while the mod is off (it pushes/pops `Rand` state, so the production random stream stays intact).
- `Vanilla` silence is unfinished content, not design: no built-in fallback profile data ships, so every pawn resolving to the built-in tier is silent. Do not re-derive it as an intentional property of the mode.
...
- Still open, not folded into this commit: the 41-section `UsHelpCatalog` remains English literals (a translation-tone decision for the maintainer, ~120 strings); container-level auto-width was NOT implemented — `UiLayoutEngine.ResolveColumnWidths` still reads only static `Width=`/equal split, so `nav-column 192` and `help-scroll 232` stay fixed and "widen for Chinese" is today achieved by growing bands, not columns; mode/preset/filter Chinese wordings (原版/回退/混音/停用, 保守/均衡/强烈/自定义, 全部/仅启用/冲突/孤立) are proposed translations awaiting maintainer review.
