...
1. 基于当前开发线切出（现为 `0.3.x`，按载主 Api 窗口命名；本仓不设常驻 dev 分支，也不留按功能命名的长期分支——改动直接落在开发线上，窗口收口时由维护者合回 `main`）。
...
1. Branch from the current development line (`0.3.x`, named after the carrier API window). There is no standing dev branch and no per-feature long-lived branch - changes land on the line directly; the maintainer merges it back to `main` when the window closes.
