...
|- Source/UniversalSqueaker/        # 主程序集（net472，0.4.0）
...
2. `About/About.xml`：packageId 固定 `coahuilite.universalsqueaker`；modVersion 与 csproj `<Version>` 一致（当前 0.4.0，与 carrier 对齐）；显示名/许可待定
...
- `Source/UniversalSqueaker/UniversalSqueaker.csproj` 已建立（Version 0.4.0 主源，与 carrier 对齐；身份轴 `<Version>`/`<VersionPrefix>`/`<modVersion>` 随发布一起抬，后缀只进 tag/工件名）；`1.6/Assemblies/` 为构建输出路径。
