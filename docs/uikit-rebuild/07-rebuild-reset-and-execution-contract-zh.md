...
> **修正（2026-09-04，历史证据保留原文）**：本文件 §1 的"两条路径"是调查期事实。现已只有一条：kernel/Schema=2 路径即生产路径，旧 `VoicePacksPage`/`FerriteVoicePacksPage`/旧 `Layout.xml` 链已在 `1a4e904` 整体删除；"未接入垂直切片"的表述不再成立。当前事实以 `HANDOFF.md` 与代码为准。
