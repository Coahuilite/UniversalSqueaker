...
> **修正（2026-09-04，历史证据保留原文；hash 于 2026-09-07 重定位）**：本文件 §1 的"两条路径"是调查期事实。现已只有一条：kernel/Schema=2 路径即生产路径，旧 `VoicePacksPage`/`FerriteVoicePacksPage`/旧 `Layout.xml` 链已在 `38b1247` 整体删除；"未接入垂直切片"的表述不再成立。§2 末"Gate U 未通过前，禁止 clean cutover"与 §C1 同批失效——clean cutover 已于 2026-09-02 执行。当前事实以 `HANDOFF.md`（维护者本地）与代码为准。
