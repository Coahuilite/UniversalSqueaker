...
| Race 域列表 | ✅ | ✅ | 无 |
| Xenotype 域列表 | ❌ 模型有、UI 无入口 | ✅ 已新增 `us/xenotype-layer` | MVP 实现已补齐 |
| 包清单 / 搜索 / 启停 | ✅ | ✅ | 无 |
| Legacy SR 标记 | ✅ | ✅ | 无 |
| 域状态 / orphan / Forget | ✅ | ✅ | 无 |
| 页面级滚动 | ✅ | ✅ | 无 |
...
真正必须补的只有一项：**Xenotype 域可见入口**。MVP 实现新增 `us/xenotype-layer` 组件与注册，Ferrite 路径已具备 Xenotype 域列表。其余差异主要是视觉/打磨。
