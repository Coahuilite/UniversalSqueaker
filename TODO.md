...
- **D9 (new defect, log-proven + maintainer-confirmed "有明显截断")**: pack-filter labels clip. Session 2 logged two `ui.text.overflow` warnings: popup item `(unscoped)` small font needs 530.0px has 131.0px; `page-root/body-row/content-scroll/filter-bar` tiny font needs 413.3px has 131.0px. The pack-filter trigger/popup column is a fixed 143px (`publish id=pack-filter rect=782.0,238.0,143.0,48.0`), far too narrow for long pack/author names. This is the G1 width-axis class arriving in game. Fix direction (awaiting go): popup width grows to content capped by the window; trigger label ellipsizes; no tooltip (C+A rule).
- **D10 (new UX request)**: the help panel flickers A -> section overview -> B when the pointer moves quickly between adjacent controls, because the window clears the claim every frame and the fallback is immediate. Request: a short grace period before releasing to the overview. Direction (awaiting go): keep the last claim for a small frame count (e.g. ~15-20 frames) before clearing; a NEW claim replaces immediately, so only the clear-to-overview path is delayed - no pin semantics return.
