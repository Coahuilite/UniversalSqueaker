...
## FerriteLib extraction (ruled 2026-09-03; in flight)

- **Phase 0 — library-side prep inside this tree: DONE (`368297e`).** Language term in the layout cache key; `VerseFerriteTextMetrics` relocated into the library and made public; unread theme tokens deleted and `AccentGoldAlpha20` made derived; `UiTheme.DarkGold` turned into a per-call template; three new harness lanes, all mutation-checked. 15/15 gates green.
- **Phase 1 — physical extraction: blocked on maintainer authorization (writes outside this repo root).** Exact remaining steps:
  1. Create the sibling repo `ferritelib` (git init) with its own `About/About.xml` (`coahuilite.ferritelib`, display name FerriteLib), `LoadFolders.xml`, `1.6/Assemblies/`, and its own AGENTS/MEMORY/TODO following this repo's protocol.
  2. Move `Source/FerriteLib.UiKit/` + `tools/FerriteLib.UiKit.Tests/` (+ 4 stub projects) out of this repo.
  3. Land `FerriteLibVersion` (hand-authored API version, never `AssemblyInformationalVersion`) + a `Require(min,max,consumerPackageId)` assert + a duplicate-carrier guard that enumerates loaded assemblies by simple name and reports each copy's owning mod and `ModVersion`.
  4. Publish as a local NuGet package (the `Krafs.Rimworld.Ref` model) so US and NGS both take a *versioned compile-time* contract.
  5. Unbind US: drop the `ProjectReference`, stage/`check-pack-readiness`/`verify-local` gate 12 stop expecting `FerriteLib.UiKit.dll`, add `modDependencies` + `loadAfter` to `About/About.xml`.
  6. Do not invent a `Languages/` folder for the extracted mod: the library has **no** player-visible string of its own today (`Ferrite.UiKit` appears nowhere in `Source/**`; `UiSessionGuard` emits only an English `[FerriteLib.UiKit] ...` log line, and every visible fallback string is drawn by the consumer's own `fallback` delegate). `01-product-and-architecture-decisions-zh.md:208` ("UiKit fallback 文本使用 Ferrite 自有翻译键") is documented intent that was never implemented — either implement it in the new repo or correct the decision, but do not gate the new repo against a key set that does not exist. `US.Overlay.ManifestMissing` stays in US.
