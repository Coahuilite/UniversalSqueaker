...
- [ ] **Branch protection on main: BLOCKED BY PLATFORM, not by choice** - private repos on the current plan cannot enable it (REST 403: "Upgrade to GitHub Pro or make this repository public"). The carrier's public repo has the minimal shape (force-push off, deletion off, no required checks). Decide together with the US visibility flip: either upgrade/protection, or public+protection, or accept the documented gap while private.
- [ ] **First US rc tag - ordering (revised 2026-09-07 after maintainer pushback)**: the earlier case for staying private rested partly on public-exposure risk, which the maintainer ruled nonexistent (SR has 0 forks / 0 stars / 0 watchers, sole contributor; ~20 asset downloads across five releases). What survives is engineering-only: release.yml has never executed a job (its one run was the 0s workflow-file rejection), and §6 reconciliation is a pre-announcement gate. Both are equally satisfiable public or private, so **visibility is now a free choice, not a sequencing constraint**. **Prerequisite either way**: a release commit setting csproj `<Version>` + About.xml `<modVersion>` from `0.1.0-dev` to the clean base the tag names (release.yml throws on `-dev` or a mismatch); the version number itself is a maintainer product decision. Tagging stays maintainer-authorized.
- [ ] **Public flip**: with exposure ruled a non-issue, the only remaining blockers are the pending Workshop display name/license copy. Flipping BEFORE the first tag is now the better option if branch protection is wanted, because protection requires public (private repos on this plan get REST 403) and the first release commits are exactly when main wants force-push protection.
