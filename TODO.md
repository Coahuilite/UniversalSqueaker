...
- **D9 (new defect, log-proven + maintainer-confirmed "有明显截断")**: pack-filter labels clip. Session 2 logged two `ui.text.overflow` warnings: popup item `(unscoped)` small font needs 530.0px has 131.0px; `page-root/body-row/content-scroll/filter-bar` tiny font needs 413.3px has 131.0px. The pack-filter trigger/popup column is a fixed 143px, far too narrow for long pack/author names. This is the G1 width-axis class arriving in game. **FIXED 2026-09-06 (`03e4868`); in-game verified 09-06b (`7524a63`): 作者下拉无截断、完整可读.**
- **D10 (new UX request)**: the help panel flickers A -> section overview -> B when the pointer moves quickly between adjacent controls, because the window clears the claim every frame and the fallback is immediate. **FIXED 2026-09-06 (`03e4868`); in-game verified 09-06b (`7524a63`): 跨控件移动不再闪回总览.**

### 09-06b closure round (package `7524a63`, maintainer report)
