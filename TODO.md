...
- [ ] **First US rc tag - ordering ruled by the US session's evaluation (2026-09-07): test rc1 while private, flip visibility after.** release.yml has never executed a job (its only run was the 0s workflow-file rejection; the duplicate-key fix is structurally verified but unproven), the §6 reconciliation is by design a pre-announcement gate, and any rc1 rework is cheap only while history is private. Private repos run the workflow and serve assets to an authenticated maintainer, so nothing in the rc1 test needs public. **Prerequisite either way**: a release commit setting csproj `<Version>` + About.xml `<modVersion>` from `0.1.0-dev` to the clean base the tag names (release.yml throws on `-dev` or a mismatch); the version number itself is a maintainer product decision. Tagging stays maintainer-authorized.
- [ ] **Public flip (maintainer decision, after rc1)**: blockers are the pending Workshop display name/license copy and whatever the rc1 round surfaces; the argument FOR flipping is branch protection, which private repos on this plan cannot enable at all.
