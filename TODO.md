...
## Help presentation redesign (2026-09-05; C+A ruled, plan delivered, implementation awaiting go)
...
- Ruled by maintainer 2026-09-05: **C+A** — hovering a control shows its component help, moving away falls back to the help of the currently selected big-level section (no pin). Implementation plan delivered same day (zero UiKit change; per-frame clear in the US window; panel height invariant to hover; 29-key bidirectional wiring guard). Open decision left: translation tone for the 41 catalog entries (proposal: manual-style, 3 samples first).
- Ruled 2026-09-05 (library read-only during GitHub release prep): the C+A implementation is **zero UiKit change** — hover claims ride the existing global `set-help-hover` binding, per-frame clearing lives in the US window. The per-frame focus-note + hover-claim mechanism is registered as a **post-thaw library candidate primitive** (`UiSession` transient, same family as `SetPopupRect`/`ScrollTarget`); promote only when a second consumer or a second per-frame transient need appears. The help panel widget itself (catalog, index, fallback resolution, translation seam) stays consumer-side permanently — it is content, not mechanism.
