...
- **Cross-repo round OPEN — FL→US round 2 (2026-09-07): packaging scripts.** Buffer section `## FL→US round 2` in `../ferritelib/HANDOFF.md`, items S1-S6; FL fixed S1 in its own tree first (`f2f4dd0`) and offered US the check that caught it. **S1-S5 executed in US the same day** (measured reproduction and controls in `HANDOFF.md` §6): flavor measured out of the payload instead of declared, dev rehearsal is a folder, one deterministic archive writer in the engine, identity/debris isolation by construction, carrier gate measures bytes. **S6 needs no code**: recorded below as a dead-until-wired axis. Round closes when FL marks it reviewed (US side is done; the verdicts are in `HANDOFF.md` §6 and this line is the pointer until then).
