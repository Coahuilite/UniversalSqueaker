...
// 触发模型纯类型（漏斗纯逻辑提取：SqueakActionPlan/SqueakTriggerInvocation → 纯文件 + harness 链接）。
// 本文件零 Verse 引用；产品元数据 SqueakActionDefinitions 与绑定 SqueakActionConfig 的工厂留在适配层
// SqueakActionModel.cs。namespace 保持根命名空间（存档按名序列化 ABI 不变）。
...
/// 身份门控纯标志：PlayerSelection/ActiveCommand 派生 IsPlayerInitiated（需玩家可控 pawn），
